...
### Playwright MCP Benchmark (Native Tool Calling via Cloud APIs)

This setup evaluates the efficiency of agent execution when using standard Playwright selectors (Human Page) versus dynamic high-level semantic tools exposed via AgentLayerWeb (Agent Page). The tests are routed through standard high-performance AI cloud model APIs.

| Model | Scenario | Duration | Steps | Total Tokens | API Cost (USD) | Speedup & Savings |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **gemini-3.5-flash-extra-low** | Human Page <br> Agent Page | 42.15s <br> **24.15s** | 15 steps <br> **11 steps** | 79,708 <br> **14,867** | $0.006142 <br> **$0.001216** | **1.75x speedup** <br> **81.3% token savings** <br> **80.2% cost reduction** |
| **gemini-3.5-flash-low** | Human Page <br> Agent Page | **24.15s** <br> 54.27s | **11 steps** <br> 12 steps | 54,544 <br> **22,854** | $0.004180 <br> **$0.001828** | 2.25x slowdown <br> **58.1% token savings** <br> **56.3% cost reduction** |
...
| **gemini-3.1-pro-low** | Human Page <br> Agent Page | 65.07s <br> **20.63s** | 15 steps <br> **6 steps** | 82,310 <br> **7,154** | $0.006327 <br> **$0.000597** | **3.15x speedup** <br> **91.3% token savings** <br> **90.6% cost reduction** |

*Note: LLM planning noise is common on lower-tier models (like flash-low) where reasoning loop variations occur.*

### Key Takeaways
1. **Dramatic Efficiency Gains**: For capable models (like `gemini-3.1-pro-low`), using high-level semantic tools generated by AgentLayerWeb yields a **3.15x speedup** and **91.3% token savings**.
2. **Minimal Steps**: Instead of wasting steps writing custom CSS selectors, dealing with browser syntax errors (e.g. `:contains`), or fetching raw page HTML repeatedly, the agent performs actions directly, resolving the form validation flow in just **6 steps** vs maxing out at **15 steps** on the raw DOM.
